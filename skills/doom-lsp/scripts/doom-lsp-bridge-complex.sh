...
main() {
    case "$1" in
        ...
        find-def)
            ...
            # 先检查符号是否存在（使用简单的 grep 检查）
            if ! grep -q "$SYMBOL" "$FILE" 2>/dev/null; then
                log_error "符号 '$SYMBOL' 在文件 $FILE 中不存在"
                return 1
            fi
            ...
            # 尝试跳转到定义（带超时）
            timeout 5s $EMACSCLIENT -e "(progn (find-file \"$FILE\") (search-forward \"$SYMBOL\" nil t) (lsp-find-definition))" >/dev/null 2>&1
            
            if [ $? -eq 124 ]; then
                log_warning "跳转操作超时（5秒）"
            else
        ...
        find-ref)
            ...
            # 尝试查找引用（带超时）
            timeout 5s $EMACSCLIENT -e "(progn (find-file \"$FILE\") (search-forward \"$SYMBOL\" nil t) (lsp-find-references))" >/dev/null 2>&1
            
            if [ $? -eq 124 ]; then
                log_warning "查找引用操作超时（5秒）"
            else
                log_success "已尝试查找引用"
            fi
            ;;
